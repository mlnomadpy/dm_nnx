...
from ..common.train import mae_pretrain_model_loop, train_model_loop
...
def run_training_and_analysis(
    ...
    """Runs a full training and analysis pipeline for the MAE model."""
    print(
    ...
    if pretrain:
        if not is_path:
            ...
            print("\n🚀 STEP 1: Starting MAE Pretraining...")
            pretrained_encoder_path = mae_pretrain_model_loop(
                model_name="mae",
    ...
    # Get class names and datasets for fine-tuning
    test_ds = None
    if is_path:
        custom_config = dataset_configs.get("custom_folder", fallback_configs)
        train_ds, test_ds, class_names_comp, _ = create_image_folder_dataset(
            train_path,
            validation_split=custom_config.get("test_split_percentage", 0.2),
            seed=42,
        )
        ...
        num_classes_comp = len(class_names_comp)
        # train_ds and test_ds will be loaded inside train_model_loop for TFDS
    ...
    def get_test_iter():
        """Creates and returns a batched test data iterator."""
        if is_path:
            config = dataset_configs.get("custom_folder", fallback_configs)
            image_size = config.get("input_dim", (32, 32))
            num_channels = config.get("input_channels", 3)
            batch_size = config.get("batch_size", 128)
            processor = get_image_processor(
                image_size=image_size, num_channels=num_channels
            )
            # The test_ds is already created for path-based datasets
            return test_ds.map(processor).batch(batch_size).as_numpy_iterator()
        else:  # TFDS logic
            config = dataset_configs.get(dataset_name, fallback_configs)
            split = config.get("test_split", "test")
            batch_size = config.get("batch_size", 64)
            image_key = config.get("image_key", "image")
            label_key = config.get("label_key", "label")

            ds = tfds.load(dataset_name, split=split, as_supervised=False)

            def preprocess(batch):
                image = tf.cast(batch[image_key], tf.float32) / 255.0
                # You might need to resize the image here if it's not already sized correctly
                # image = tf.image.resize(image, [height, width])
                return {"image": image, "label": batch[label_key]}

            return ds.map(preprocess).batch(batch_size).as_numpy_iterator()

    print("\n📊 STEP 3: Running Analysis...")
    plot_training_curves(metrics_history, "ViT")
    print_final_metrics(metrics_history, "ViT")
    predictions_data = detailed_test_evaluation(
        model, get_test_iter(), class_names=class_names_comp, model_name="ViT"
    )
    if num_classes_comp <= 50:
        plot_confusion_matrix(predictions_data, "ViT")
...
def main():
    ...
    run_training_and_analysis(
        ...
        learning_rate=1e-3,
        momentum=0.9,
        pretrain=True,  # Set to True to run MAE pretraining
    )
